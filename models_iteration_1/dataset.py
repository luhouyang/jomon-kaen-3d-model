...
class PreprocessJomonKaenDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Shuffled points here are for the initial sampling to make sure each epoch data is different
        shuffled_indices = np.random.permutation(len(pottery_points))
        shuffled_pcd = o3d.geometry.PointCloud()
        shuffled_pcd.points = o3d.utility.Vector3dVector(pottery_points[shuffled_indices])
        shuffled_pcd.colors = o3d.utility.Vector3dVector(pottery_colors[shuffled_indices])
        ...
        if self.n_samples_global <= self.num_local_seeds:
            ...
            pcd_seeds = pcd_fps_global.farthest_point_down_sample(self.num_local_seeds)
        ...
        if len(unique_local_indices) > 10:
            ...
            rand_start_idx = np.random.randint(0, len(pottery_points) - self.n_samples_local)
            patch_indices = np.arange(rand_start_idx, rand_start_idx + self.n_samples_local)
            local_patch_pcd = pottery_pcd.select_by_index(patch_indices)

        pcd_fps_local = local_patch_pcd.farthest_point_down_sample(self.n_samples_local)
