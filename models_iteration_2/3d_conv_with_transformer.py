...
class JomonKaenVoxelDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if not train_data:
            print(
                "WARNING: The training dataset is empty. Might be because all pottery found is in test group!"
            )
        ...
        if stage in ('fit', None) and self.hparams.use_weighted_loss:
            ...
            train_labels_df = all_labels_df[all_labels_df['CODE'].isin(
                train_files_for_lookup
            )]  # Because the CODE column is "POTTERY.ply"
...
class Conv3DNetwork(nn.Module):
    ...
    def __init__(self,
        ...
        self.feature_extraction = nn.Sequential(*[
            self.conv_block(in_dim,
                            conv_dims[i + 1])
            for i, in_dim in enumerate(conv_dims[:-1])
        ])

        final_size = resolution // (2**(len(conv_dims) - 1))

        self.pos_encoder = PositionalEncoding(conv_dims[-1])
        self.transformer_block = TransformerBlock(conv_dims[-1],
                                                  transformer_heads,
                                                  transformer_ff_dim)
    ...
    def forward(self, x):
        ...
        b, c, d, h, w = x.shape
        x = x.view(b, c, -1).permute(2, 0, 1)  # Reshape for transformer
        x = self.pos_encoder(x)
        x = self.transformer_block(x)
        x = x.permute(1, 2, 0).view(b, c, d, h, w)  # Reshape back
        x = self.flatten(x)
...
class SaveValidationPredictions(Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        is_interval_save = (epoch > 0
                            and epoch % self.save_every_n_epochs == 0)
        is_best_loss_save = (current_val_loss is not None
                             and current_val_loss < self.best_val_loss)

        if is_interval_save:
            self._save_to_file(pl_module,
                               filename=f"eval_predictions_spoch_{epoch}.csv")
